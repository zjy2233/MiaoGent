...
DISCOVER_PROMPT = """从以下对话中提取关于用户的任何事实信息，包括但不限于：
- 身份与角色（姓名、职业、职位、身份标识等）
- 与他人的关系（如"我是你的创建者"、"我的同事是..."）
- 兴趣、偏好、习惯
- 所在地、联系方式
- 知识背景、技能
- 任何用户主动透露的个人信息
...
只返回你有把握的信息，不要猜测。
以 JSON 格式返回，key 使用中文或英文描述信息类别，value 是具体内容。
如果没有可提取的新信息，返回空对象 {{}}。
格式示例：{{"name": "张三", "occupation": "工程师", "role": "MiaoGent 的创建者"}}
