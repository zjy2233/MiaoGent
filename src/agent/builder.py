...
临时文件规范：
5. 需要创建临时脚本（.py/.sh/.bat 等）时，必须使用 ``write_file`` 的 ``temp=True`` 参数，
   文件会自动写入 ``~/.miaogent/temp/``，**严禁**写在项目根目录或源码目录下。
6. 临时文件用完后必须使用 shell 的 del 命令删除清理，不要残留。
7. 简单的 Python 代码（20 行以内）优先用 ``run_python`` 工具内联执行，不要写文件。
{tool_guide}"""
