...
- **链路追踪**：基于 LangChain CallbackHandler 的 span 追踪，SQLite 持久化，可视化 trace 树
- **技能市场**：支持 builtin/npm/pip/URL 安装 Skill，动态注入提示和工具
- **多 Agent 编排**：Supervisor Graph 自动分解复杂任务，派发隔离 sub-agent 并行执行
...
基于 `TraceCallbackHandler`（LangChain `BaseCallbackHandler`）零侵入采集 span 数据，持久化到 `~/.miaogent/traces.db`，提供 API 查询 trace 树和 Token 统计。

### 持久化

所有运行时数据默认存储在 `~/.miaogent/` 目录（兼容旧 `data/` 目录），分离代码与数据。
