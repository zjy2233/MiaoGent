...
const BALL_W = 148;
const BALL_H = 155;
const BALL_W_EXPANDED = 340;
const BALL_H_EXPANDED = 210;
const PANEL_W = 420;
const PANEL_H = 520;
...
ipcMain.on('set-window-shape', (_event, rects) => {
  if (!ballWindow) return;
  try {
    ballWindow.setShape(rects && rects.length > 0 ? rects : []);
  } catch (err) { console.error('[agent-shell] setShape error:', err); }
});
